...
class MPVGTKManager(Gtk.Window):
    def __init__(self):
        ...
        self.list_store = Gtk.ListStore(str, int, str, str, str, str, str)
    ...
    def _update_thread(self):
        ...
        for idx, i in enumerate(res["data"]):
            name = i.get("title") or os.path.basename(i.get("filename", "Unknown"))
            grp = self.m3u_groups.get(name, "Unkategorisiert")
            groups.add(grp)
            items.append({"name": name, "orig_idx": idx, "group": grp})
        def sort_priority(x):
    ...
    def _finalize_update(self, groups, full_sorted):
        ...
        for i in full_sorted:
            ...
            it = self.list_store.append([dn, i["orig_idx"], "800" if is_p else "400", i["group"], "media-playback-start" if is_p else None, fg, bg])
