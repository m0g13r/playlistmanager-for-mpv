...
class MPVGTKManager(Gtk.Window):
    def __init__(self):
        ...
        self.sort_mode, self.current_playing_path, self.current_group, self.is_updating, self.resume_done, self.last_file_path = 0, "", "All", False, False, ""
        self.apply_css()
    ...
    def _finalize_update(self, groups, full_sorted, curr_p):
        ...
        for i in full_sorted:
            ...
            dn = (f"▶ " if is_p else "") + (f"★ " if is_f else "") + i['name']
    ...
    def filter_func(self, model, iter, data):
        dn = model.get_value(iter, 0); name, grp, q = dn.replace("★ ", "").replace("▶ ", ""), model.get_value(iter, 3), self.search_entry.get_text().lower()
    ...
    def on_click(self, tree, event):
        ...
        if event.button == 1: self.activate_row(pi[0])
            ...
            if f_iter:
                n = self.filter.get_value(f_iter, 0).replace("★ ", "").replace("▶ ", "").strip()
    ...
    def save_window_state_now(self):
        try:
            path_res = self.send_command({"command": ["get_property", "path"]})
            curr, pos, size = path_res.get("data", "") if path_res else "", self.get_position(), self.get_size()
            with self.file_lock:
