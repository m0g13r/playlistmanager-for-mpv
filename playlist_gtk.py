...
class MPVGTKManager(Gtk.Window):
    def __init__(self):
        ...
        self.vol_scale = Gtk.Scale(orientation=Gtk.Orientation.VERTICAL, adjustment=adj)
    ...
    def apply_css(self):
        ...
        .fab-trigger { min-width: 32px; min-height: 32px; background: rgba(53, 132, 228, 0.7); color: white; } 
        .fab-trigger:hover { background: rgba(53, 132, 228, 0.9); } 
        .fab-small { min-width: 28px; min-height: 28px; background: rgba(60, 60, 60, 0.6); color: white; } 
        .fab-small:hover { background: rgba(80, 80, 80, 0.8); }
        .fab-vol-slider { background: rgba(60, 60, 60, 0.6); border-radius: 14px; padding: 12px 0; }
        scale.fab-vol-slider contents trough { background: rgba(255, 255, 255, 0.2); min-width: 4px; border-radius: 2px; margin: 0 12px; }
        scale.fab-vol-slider contents trough highlight { background: #3584e4; border-radius: 2px; }
        scale.fab-vol-slider contents trough slider { background: #3584e4; min-width: 12px; min-height: 12px; border-radius: 50%; margin: -4px; border: none; box-shadow: none; }
        treeview { background-color: transparent; }
        treeview selection { border-radius: 8px; }
        treeview:selected { border-radius: 8px; background-color: #3584e4; color: white; }
        """
        p = Gtk.CssProvider()
    ...
    def send_command(self, cmd):
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as c:
                c.settimeout(1.0); c.connect(self.socket_path); c.sendall(json.dumps(cmd).encode() + b"\n")
                res = b""
                while True:
                    chunk = c.recv(8192)
                    if not chunk: break
                    res += chunk
                    if res.endswith(b"\n"): break
                if res: return json.loads(res.decode(errors="ignore").splitlines()[0])
        except: return None
    ...
    def _finalize_update(self, groups, full_sorted, curr_p):
        self.list_store.clear(); self.full_list_data = full_sorted; active_iter = None; self.current_playing_path = curr_p
        with self.favorites_lock: fav_copy = set(self.favorites)
        for i in full_sorted:
            is_p = i["filename"] == curr_p; is_f = i["name"] in fav_copy; dn = f"★ {i['name']}" if is_f else i['name']
            if is_p: dn = f"▶ {dn}"
    ...
    def rebuild_group_menu(self, groups):
        ...
        for o in ["All", "★ Favorites"] + sorted(list(groups)):
            lbl = f"{o} ({counts.get(o, 0)})"; item = Gtk.MenuItem(label=f"• {lbl}" if o == self.current_group else lbl); item.connect("activate", self.on_group_selected, o); self.group_menu.append(item)
    ...
    def update_now_playing(self):
        path_res = self.send_command({"command": ["get_property", "path"]})
        if path_res and "data" in path_res and path_res["data"] != self.current_playing_path: self.current_playing_path = path_res["data"]; self.update_playlist()
    ...
    def filter_func(self, model, iter, data):
        dn = model.get_value(iter, 0); name = dn.replace("★ ", "").replace("▶ ", ""); grp = model.get_value(iter, 3); q = self.search_entry.get_text().lower()
    ...
    def on_load_clicked(self, mi):
        diag = Gtk.FileChooserDialog(title="Playlist", parent=self, action=Gtk.FileChooserAction.OPEN); diag.add_buttons("_Cancel", Gtk.ResponseType.CANCEL, "_Open", Gtk.ResponseType.OK)
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, "r", encoding="utf-8") as f:
                    c = json.load(f); self.move(c.get("x", 100), c.get("y", 100)); self.resize(c.get("w", 200), c.get("h", 750)); self.current_group = c.get("current_group", "All"); self.last_file_path = c.get("last_playing", "")
        except: pass
    def save_window_state_now(self):
        try:
            path_res = self.send_command({"command": ["get_property", "path"]}); curr = path_res.get("data", "") if path_res else ""; pos = self.get_position(); size = self.get_size()
            with self.file_lock:
                with open(self.config_file, "w", encoding="utf-8") as f: json.dump({"x": pos[0], "y": pos[1], "w": size[0], "h": size[1], "current_group": self.current_group, "last_playing": curr}, f)
        except: pass
